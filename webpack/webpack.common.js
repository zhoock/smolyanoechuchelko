...
// Снимок env для клиентского бандла (DefinePlugin подставляет литерал на этапе сборки)
const envForBundle = { ...process.env };
if (envForBundle.NODE_ENV === undefined) {
  envForBundle.NODE_ENV = production ? 'production' : 'development';
}
if (envForBundle.NETLIFY_SITE_URL === undefined) {
  envForBundle.NETLIFY_SITE_URL = '';
}
...
module.exports = {
  ...
  plugins: [
    ...
    // Клиентский код читает process.env.*; значения фиксируются на момент сборки
    new webpack.DefinePlugin({
      'process.env': JSON.stringify(envForBundle),
    }),
  ],
